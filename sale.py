...
class addSale(Tk):
    def __init__(self):
        ...
        self.selectId = Label(self, text="Enter the date in the format dd-mm-yyyy")
    ...
    def sale(self):
        try:
            ...
            qry="insert into sales set consumerName='%s', address='%s', paymentMode='%s', mobileNumber=%d, paid=%d" %(self.consumerName.get(),self.address.get(),self.paymentMode.get(),int(self.mobileNumber.get()),int(self.paid.get()))
            cur = con.cursor()
            cur.execute(qry)
